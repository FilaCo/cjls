...
- **`modules/loupe`** (`static`) — the analysis, knowing no LSP ([D5](docs/adr/0005-loupe-knows-no-lsp.md)): `loupe.vfs` (`FileId`, `VfsPath`, `PathInterner`, `Vfs`), `loupe.db` (`@CalcaInput SourceFile`, `RootDatabase`), `loupe.syntax` (the `parse` query), and the API in `loupe` (`fileStructure`). Depends on `calca`, `cjsyntax`, `ginkgo`, `index_map`, `rope`. See [02-analysis.md](docs/design/02-analysis.md).
...
- Queries take `RootDatabase` itself (A1), are keyed by entities, never positions (A2), and return `Equatable` values (A10).
- Lines break at `\n` only; offsets are UTF-8 bytes on character boundaries (A6, A7).
- The server's `Database` owns the `Vfs`, the open documents, the negotiated `Encoding` and the `RootDatabase` as `analysis`; a `DatabaseSnapshot` holds a snapshot of it, cancelled by the request's token (D8). Neither is a database itself: the two types keep a `readonly` handler from writing at compile time.
