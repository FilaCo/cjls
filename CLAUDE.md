...
Eleven members declared in the root `cjpm.toml`, each its own package.
...
- **`modules/cjsyntax`** (`static`) — the Cangjie lexer and parser, on `ginkgo`: `tokenize(text)`, `parse(text)`, and `SyntaxKind`.
...
Dependencies flow one way ([00-layers.md](docs/design/00-layers.md)): `cjls → jsonrpc → stdxx`; `cjls → loupe → {calca → index_map, cjsyntax → ginkgo, rope}`. `lsp_codegen` sits outside that chain, on `cjtoml` (which depends on nothing) and `stdxx`.
