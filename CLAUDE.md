...
- **`modules/cjls`** (`executable`) — the server: entrypoint and logging (`cjls`), the handlers (`cjls.handlers`), the framework they run in (`cjls.server`) with its `@Handler` macro (`cjls.macros`), and the generated `cjls.lsp_types`. Depends on `jsonrpc`.
...
**Handlers take a context and their params, and return the result, registered the way axum does it:**
