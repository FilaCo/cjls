...
- **`modules/cjls`** (`executable`) — the server: entrypoint and logging (`cjls`), the handlers (`cjls.handlers`), the framework they run in (`cjls.server`), and the generated `cjls.lsp_types`. Depends on `jsonrpc`.
...
`main.cj` builds a `SimpleLogger` on stderr (`logging.cj`; level from `CJLS_LOG_LEVEL`, default `INFO`) and serves stdio with `serve(transport, router(), logger)`, exiting with the code the protocol asks for — `0` only for an `exit` after a `shutdown`. There is no global logger: it is passed down, and never print — stdout is the LSP wire. How to add a request is in `CONTRIBUTING.md`.

**Handlers are plain functions of their params and what they ask for, registered the way axum does it — no macros:**
...
- **The spec value carries the types.** `route` infers `P` and `R` from the spec, so a handler of the wrong params or result fails to compile with an unsolvable-constraint error, and a lambda's params need no annotation. A spec-only type argument (`route<S>`) is not an option: Cangjie takes all of a function's type arguments or none.
- **Extractors** (`server/context.cj`), as axum's `FromRequestParts`: before its params a handler takes up to three of `Database` / `DatabaseSnapshot`, `Client`, `CancellationToken` (never cancelled for a notification), `Logger` (already carrying `method` and `id`), or the whole `Context` / `ReadOnlyContext`. A type is an extractor by implementing `FromContext<T>` / `FromReadOnlyContext<T>`, whose static `from` the router calls through the type parameter. There is one overload per count because Cangjie has no variadic generics, and a function type can't be `extend`ed. In a lambda, the extractors need annotating (`{client: Client, params => …}`); the params don't.
- **`route` vs `readonly`** (`server/router.cj`) is how a handler runs: `route` on the read loop, in arrival order, with the `Database` — how inputs change; `readonly` on its own `spawn` with a `DatabaseSnapshot` taken on the read loop when the message arrived. Which one a handler is written for is in what it takes: `Database` only implements `FromContext`, so asking for it under `readonly` does not compile. `Database` is a placeholder for the query database, already split the way it will be; `.withState(...)` supplies it.
- **Registration is explicit**, one line per handler in `handlers/router.cj`, as in rust-analyzer; `Router` refuses a second handler for a method, and `readonly` for `initialize`, `shutdown` or `exit`, since whatever follows them must see the state they leave.
