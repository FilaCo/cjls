...
Ten members declared in the root `cjpm.toml`, each its own package.

**What is a module and what is a package.** A package is Cangjie's unit of compilation (a crate of rust-analyzer's maps to a package, not a module), so splitting code into modules buys no build parallelism. A module is for a *library* — code that knows nothing of the server or of LSP, with its own dependencies and `output-type`, which something other than `cjls` could use (`cjsyntax` knows Cangjie, but not LSP). Whatever only the server has is a package of `cjls`: nothing can depend on an executable module anyway, and its packages can share `protected` declarations (visible module-wide) rather than `public` ones. Should a second executable need the analysis one day, its packages move out into a static module then.
...
- **`modules/rope`** (`static`) — `Rope`, an immutable UTF-8 text as a B-tree of chunks, and the line/column arithmetic LSP needs (`Encoding`, `LineColumn`, `TextSummary`). No project dependencies. See *`rope`, `cjls.vfs` and `cjls.db`* under Architecture.
...
- **`modules/cjls`** (`executable`) — the server: entrypoint and logging (`cjls`), the handlers (`cjls.handlers`), the framework they run in (`cjls.server`) with its `@LspHandler` macro (`cjls.macros`), the generated `cjls.lsp_types`, and the analysis under them: `cjls.vfs` (rust-analyzer's `vfs`: `FileId`, `VfsPath` from and to URIs, `PathInterner`, and `Vfs`, the files' contents with the changes since they were last taken) and `cjls.db` (the inputs on `calca`: `@CalcaInput SourceFile` and `RootDatabase`). Depends on `jsonrpc`, `calca`, `index_map` and `rope`.
...
Dependencies flow one way: `cjls → jsonrpc → stdxx`; `cjls → {calca → index_map, index_map, rope}` and `cjsyntax → ginkgo`; inside `cjls`, `server → db → vfs`. `lsp_codegen` sits outside that chain, on `cjtoml` (which depends on nothing) and `stdxx`.
...
### `rope`, `cjls.vfs` and `cjls.db` — the files, as inputs of the analysis

rust-analyzer's split, piece for piece: `vfs` knows files, `db` (its `base-db`) makes them inputs, the server keeps the two in step.

- **`Rope`** is immutable and persistent: an edit (`replace(range, text)`, `insert`, `remove`, `slice`) rebuilds only the path to the chunks it touches, so the rope before it stays valid and shares the rest — what an input's old value and a snapshot need. Leaves hold at most `MAX_CHUNK` bytes cut on character boundaries; every node but the root holds at least half of what it may, merged on the way back up an edit (`checkInvariants` is what the tests hold it to). Each node caches a `TextSummary` — bytes, UTF-16 units, characters, `\n`s — so every lookup by any measure goes straight down. **Only `\n` breaks a line**, as in rust-analyzer and in the compiler's lexer (and `cjsyntax`'s), which take `\n` and `\r\n` as line ends and a lone `\r` as none: a `\r\n` numbers lines the same, and `lineEnd` stops before its `\r`, so a column past the line never lands inside it. `offsetOf(LineColumn, Encoding)` is lenient the way LSP asks (a column past its line is the line's end, one inside a character its start, a line past the text the text's end); offsets passed in must be character boundaries, or it throws.
- **`Vfs`** holds each file's `?Rope` by `FileId` and folds what happens to it until `takeChanges` (created then deleted is nothing; setting the same text is nothing). A `FileId` is its path's index in the `PathInterner` (an `IndexSet`), for good; the changes are an `IndexMap` of file to kind, in order; the interner is the one thread-safe part, shared with snapshots so a handler off the read loop can resolve a URI. `VfsPath.fromUri` decodes `file:` URIs into normalized paths (so two spellings are one file) and keeps any other scheme as a virtual path; POSIX paths only for now.
- **`db`**: one `@CalcaInput SourceFile { fileId, text: Rope }` per file, made the first time `RootDatabase.setFileText` sees the id and never dropped (a deleted file is an empty text); the `FileId → SourceFile` map is shared by a database and its snapshots. **Queries take `RootDatabase` itself**, in every layer, with no database interface in between (no salsa-style `SourceDatabase <: Database` ladder): rust-analyzer needs one trait per crate because its concrete database, at the top, has to hold every query's storage, while calca keeps all of that in `Storage`, so the concrete class knows no query and sits under all of them. An interface pays only once there is a second implementation. (`Parse` is `Equatable`, tree and errors, so a parse query will backdate.)
- **In the server**, `Database` owns the `Vfs`, the open documents with their versions, the negotiated `Encoding` (UTF-8 if the client offers it, else UTF-16), and the `RootDatabase`, as `analysis`; a `DatabaseSnapshot` holds a snapshot of it, as `analysis` too, so a query takes `db.analysis`. Neither is itself a database: the two types are what keeps a `readonly` handler from writing at compile time. `setFileContents` sets the `Vfs` and, if anything changed, applies its changes to the database, which cancels what the snapshots have in flight. The sync handlers are in `handlers/document_sync.cj`: `didChange` applies a batch in order and drops it whole if a range runs backwards; `didClose` puts back what the disk holds, or removes the file.
