...
Dependencies flow one way: `cjls → jsonrpc → stdxx`; `cjls → {calca → index_map, cjsyntax → ginkgo, index_map, rope}`; inside `cjls`, `server → db → vfs`. `lsp_codegen` sits outside that chain, on `cjtoml` (which depends on nothing) and `stdxx`.
