...
- **`modules/rope`** (`static`) — `Rope`, an immutable UTF-8 text as a B-tree of chunks, and the line/column arithmetic LSP needs (`Encoding`, `LineColumn`, `TextSummary`). No project dependencies. See *`rope`, `vfs` and `base_db`* under Architecture.
- **`modules/vfs`** (`static`) — rust-analyzer's `vfs`: `FileId`, `VfsPath` (from and to URIs), `PathInterner`, and `Vfs`, the files' contents with the changes since they were last taken. Depends on `rope`.
- **`modules/base_db`** (`static`) — the analysis's inputs on `calca`: `@CalcaInput SourceFile`, `SourceDatabase`, `RootDatabase`, and the `parse` query. Depends on `calca`, `cjsyntax`, `ginkgo`, `rope`, `vfs`.
...
- **`modules/cjsyntax`** (`static`) — the Cangjie lexer and parser, on `ginkgo`: `tokenize(text)`, `parse(text)`, and `SyntaxKind`. Reaches `cjls` through `base_db`'s `parse` query.
- **`modules/cjls`** (`executable`) — the server: entrypoint and logging (`cjls`), the handlers (`cjls.handlers`), the framework they run in (`cjls.server`) with its `@LspHandler` macro (`cjls.macros`), and the generated `cjls.lsp_types`. Depends on `jsonrpc`, `base_db`, `vfs`, `rope`, and `calca` (for `Cancelled`).
...
Dependencies flow one way: `cjls → jsonrpc → stdxx`; `cjls → base_db → {calca → index_map, cjsyntax → ginkgo, vfs → rope}`. `lsp_codegen` sits outside that chain, on `cjtoml` (which depends on nothing) and `stdxx`.
...
### `rope`, `vfs` and `base_db` — the files, as inputs of the analysis

rust-analyzer's split, piece for piece: `vfs` knows files, `base_db` makes them inputs, the server keeps the two in step.
...
- **`base_db`**: one `@CalcaInput SourceFile { fileId, text: Rope }` per file, made the first time `RootDatabase.setFileText` sees the id and never dropped (a deleted file is an empty text); the `FileId → SourceFile` map is shared by a database and its snapshots. `parse(db, file)` is a `@CalcaTracked` query, backdated because `Parse` is `Equatable` (tree and errors).
