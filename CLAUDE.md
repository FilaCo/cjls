...
`cjls` is a Language Server Protocol implementation for the **Cangjie** language, written in Cangjie itself. It is an early-stage MVP, built bottom-up: the `jsonrpc` peer, the generated protocol types (messages included), and a handler framework with the lifecycle (`initialize`/`shutdown`/`exit`) are in place and tested. The server keeps the editor's documents in sync (open/close, incremental changes, the position encoding negotiated) and answers `textDocument/documentSymbol`. Under it is `loupe`, the query-driven incremental analysis on `calca`: the files (`rope`, `loupe.vfs`) as inputs (`loupe.db`), the parser (`cjsyntax`, on the `ginkgo` trees) as a query, and an API free of LSP.

**[`docs/`](docs/README.md)** holds the rules (`design/`: tables, no prose), the decisions (`adr/`: one per file, superseded, never rewritten) and the open questions (`backlog.md`). Update them in the same change as the code they describe; a rule the code no longer follows is a bug in one of the two. Rules and decisions have ids (`S3`, `A2`, `D5`) — cite them. There is no DESIGN.md: long prose went stale faster than the code, which is why the old one was removed.
...
- **`modules/ginkgo`** (`static`) — rowan for Cangjie, language-agnostic, no project dependencies. `ginkgo`: lossless green/red syntax trees (`GreenNode`/`GreenToken`, `NodeCache`, `GreenNodeBuilder`, `SyntaxNode`/`SyntaxToken`/`SyntaxElement`, `TextRange`). `TokenSet`, a set of kinds as a bitset over their `ordinal`. `AstNode` and `astChild`/`astChildren`/`astNthChild`/`astToken`, rowan's `ast` support, which generated typed views are made of. `ginkgo.parsing`: the grammar-agnostic half of rust-analyzer's parser — `Lexed`, `Input`, the event-based `Parser` with `Marker`s, and `buildTree`.
...
- **`modules/index_map`** (`static`) — `IndexMap` and `IndexSet`, a hash map and a hash set that keep insertion order and so give each entry an index, after Rust's `indexmap`; they implement `std.collection.Map` and `Set`, and `IndexSet` is an `IndexMap` to `Unit`. Anything that interns — gives values dense ids in the order first seen — is an `IndexSet`. No project dependencies.
...
Dependencies flow one way ([00-layers.md](docs/design/00-layers.md)): `cjls → jsonrpc → stdxx`; `cjls → loupe → {calca → index_map, cjsyntax → ginkgo, rope}`. `lsp_codegen` and `syntax_codegen` sit outside that chain, on `cjtoml` (which depends on nothing) and `stdxx`.
...
`main.cj` builds a `SimpleLogger` on stderr (`logging.cj`; level from `CJLS_LOG_LEVEL`, default `INFO`) and serves stdio with `serve(transport, router(), logger)`, exiting with the code the protocol asks for — `0` only for an `exit` after a `shutdown`. There is no global logger: it is passed down, and never print — stdout is the LSP wire. How to add a request is in `CONTRIBUTING.md`. Stdin is `StdinStream` (`stdio.cj`), `read(2)` on fd 0 through FFI (`_read` on Windows) — **never `std.env.getStdIn()`**: its `read` returns only when the buffer is full or at EOF (even a one-byte buffer stalls), so the server would sit on a request an editor already sent. **Nor may the read block in C** (C5): it waits in 50 ms `poll` slices (`PeekNamedPipe` and a Cangjie `sleep` on Windows) and reads only once there is input, because the runtime cannot finish a garbage collection while a thread sits in a foreign call — a `read(2)` left blocked in an idle editor kept the collector at 100% CPU and every handler stopped at its next allocation, until the client next wrote. On Windows `useBinaryStdio()` also switches fd 0 and 1 out of text mode, whose `\r\n` would break `Content-Length`. No unit test sees any of this — they run on `FakeTransport` — which is what `tests/e2e` and the editor smoke tests are for.
