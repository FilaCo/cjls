...
| Q20 | The JSON codec | `jsonrpc` and `lsp_types` go through `stdx.serialization` (`DataModel`, `JsonValue`): 1.6 ms and 4.1 MiB for a 550-symbol answer. `fjson` (#9, #31) reads and writes the same messages 2–7× faster and allocates nothing once its reader and writer are kept per connection. Before the server reads a client's bytes with it: JSONTestSuite, a differential against stdx, fuzzing; then `@DeriveExt[JsonSerializable]` for it, and the `Handler` boundary as readers and writers (an ADR) |
