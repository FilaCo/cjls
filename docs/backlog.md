...
| Q20 | The JSON codec | `jsonrpc` and `lsp_types` go through `stdx.serialization` (`DataModel`, `JsonValue`); `fjson` is what they move onto (#9, #31; `FjsonBench`, `FjRpcBench`). Before the server reads a client's bytes with it: JSONTestSuite, a differential against stdx, fuzzing; then `@DeriveExt[JsonSerializable]` for it, and the `Handler` boundary as readers and writers (an ADR) |
